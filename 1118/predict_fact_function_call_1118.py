...
class SentencingPredictor:
    ...
    def identify_crime_type(self, defendant_info, case_description):
        ...
        embezzlement_keywords = ["职务侵占", "挪用资金", "侵占公司财产"]
...
3. 被告人人身情况与前科
- 是否有前科? 前罪罪名、判刑种类、执行完毕或假释时间?
- 能否构成累犯(在前罪刑罚执行完毕或赦免后5年内,再犯应判处有期徒刑以上刑罚之罪)?
- 只要案件已经认定累犯，量刑环节就只以累犯情节从重，不再把同一前科作为独立从重情节叠加
...
- "多次诈骗"
- "多次犯罪"
- "犯罪对象为弱势群体"  # 针对老年人、残疾人、未成年人等实施犯罪
- "重大灾害期间犯罪"    # 在自然灾害、事故灾难等重大灾害期间实施犯罪

7. 法定减轻标签:
- 如果存在"自首、立功/重大立功、未成年人犯罪、从犯/胁从犯、主犯、犯罪预备、中止、未遂、防卫过当"等任意一种,请在输出数组中额外加入一个标签: "法定减轻"。

【严格限制】
- 不能凭空创造新的标签文本,只能使用上面给出的模版/固定短语。
- 金额、次数等数字必须与案情原文完全一致。
...
"""
    ...
    def _get_amount_standards_for_prompt(self, crime_type, region):
        ...
        """
        # 导入计算器中的标准
        from cal import SentencingCalculator
...
**步骤2: 分析和分层情节**
从上述情节中,识别:
- **第一层面情节(连乘)**: 未成年人、从犯、胁从犯、主犯、犯罪预备、犯罪中止、犯罪未遂
- **第二层面情节(加减)**: 累犯、前科、自首、坦白、立功、认罪认罚、退赔、取得谅解、多次盗窃、多次诈骗、多次犯罪、入户盗窃、携带凶器盗窃、扒窃、电信网络诈骗、犯罪对象为弱势群体、灾害期间犯罪等

**标准调节比例参考:**

【法定从重情节】
- 累犯: 1.1-1.4 (增加10%-40%，根据情节严重程度选择)

【酌定从重情节】
- 前科: 1.05(增加5%)
- 犯罪对象为弱势群体: 1.1 (增加10%)
- 重大灾害期间犯罪: 1.2 (增加20%)
- 多次盗窃/多次诈骗/多次犯罪: 1.15 (增加15%)
- 入户盗窃: 1.15 (增加15%)
- 携带凶器盗窃: 1.2(增加20%)
- 扒窃: 1.1(增加10%)
- 电信网络诈骗: 1.25 (增加25%)
...
- 注意：第一层面和第二层面情节需要以如下格式传入：
  第一层面: [{{"name": "从犯", "ratio": 0.8}}]
  第二层面: [{{"name": "自首", "ratio": 0.8}}, {{"name": "认罪认罚", "ratio": 0.9}}, ...]
...
def main():
    ...
    output_file = "submission_with_tools_fact_1118_with4.jsonl"
