...
class SentencingCalculator:
    ...
    def calculate_base_sentence(crime_type: str, amount: float = None,
                                ...
                                theft_count: int = None, fraud_count: int = None) -> int:
        """
        计算基准刑（单位：月）- 改进版
        ...
        if crime_type == "盗窃罪":
            ...
            if amount is None:
                base = 12
            elif amount <= 0:  # 金额为0或负数时，基准刑为12个月
                base = 12
            ...
            if amount < fraud_standards["large"]:
                ...
                base = min(int(base + additional), 120)
            else:  # 数额特别巨大
                base = 120
            ...
            # 如果是诈骗罪且诈骗次数大于3次，每多2次增加1个月基准刑
            if fraud_count is not None and fraud_count > 3:
                additional_months = int((fraud_count - 3) / 1)  # 每多1次增加1个月
                base += additional_months
